 ...
 *
 * Provider-specific HTTP details stay here. Credentials must be supplied at
 * runtime (for example from Replit Secrets / production secret storage).
 * No write operations are intentionally exposed in this first slice.
 */
...
export function createShopifyAdapter({
  ...
  return {
    async getConnectionStatus() {
      try {
        ...
        return {
          ...
          scopes: [],
          checkedAt: now(),
          capabilities: { products: true, inventory: true, orders: true },
        };
      } catch (error) {
        return {
          connected: false,
          ...
          scopes: [],
          checkedAt: now(),
          capabilities: { products: false, inventory: false, orders: false },
    ...
    async listProducts(cursor) {
      const query = cursor ? `?limit=50&page_info=${encodeURIComponent(cursor)}` : "?limit=50";
      const data = await request(`/products.json${query}`);
      return {
    ...
    async listInventory(cursor) {
      const query = cursor ? `?limit=50&page_info=${encodeURIComponent(cursor)}` : "?limit=50";
      const data = await request(`/inventory_levels.json${query}`);
      return {
        items: (data.inventory_levels ?? []).map((item) => ({
          externalId: String(item.inventory_item_id),
          sku: null,
          quantity: item.available ?? null,
    ...
    async listOrders(params = {}) {
      const query = new URLSearchParams({
        ...
        limit: String(Math.min(Number(params.limit) || 50, 50)),
      });
      const data = await request(`/orders.json?${query.toString()}`);
      return {
        items: (data.orders ?? []).map((order) => ({
          externalId: String(order.id),
          status: order.fulfillment_status ?? order.financial_status ?? null,
    ...
    mapProductId(externalId) {
      return { provider: "shopify", externalId: String(externalId) };
    },
  };
}
